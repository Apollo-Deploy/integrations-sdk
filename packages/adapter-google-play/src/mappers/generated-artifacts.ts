...
interface RawVariant {
  targeting?: {
    sdkVersionTargeting?: { value?: Array<{ min?: number }> };
    abiTargeting?: { value?: Array<{ alias?: string }> };
    screenDensityTargeting?: { value?: Array<{ densityAlias?: string; densityDpi?: number }> };
  };
  apkSet?: RawApkSet[];
...
function mapArtifactDescription(raw: RawApkDescription): GeneratedArtifactDescription {
  return {
    path: raw.path ?? '',
    splitId: raw.splitApkMetadata?.splitId ?? raw.instantApkMetadata?.splitId ?? raw.assetSliceMetadata?.splitId,
    isMasterSplit: raw.splitApkMetadata?.isMasterSplit ?? raw.instantApkMetadata?.isMasterSplit,
...
function mapModule(raw: RawApkSet): GeneratedArtifactModule {
  return {
    ...
    moduleType: (raw.moduleMetadata?.moduleType as GeneratedArtifactModule['moduleType']) ?? undefined,
    deliveryType: (raw.moduleMetadata?.deliveryType as GeneratedArtifactModule['deliveryType']) ?? undefined,
    ...
    targeting: targeting
      ? {
          sdkVersion: targeting.sdkVersionTargeting?.value?.[0]
            ...
            : undefined,
          abi: targeting.abiTargeting?.value?.map((a) => a.alias ?? 'UNSPECIFIED_CPU_ARCHITECTURE'),
          screenDensity: targeting.screenDensityTargeting?.value?.map((d) =>
...
function mapAssetSliceSet(raw: RawAssetSliceSet): GeneratedArtifactAssetSliceSet {
  return {
    moduleName: raw.assetModuleMetadata?.name ?? '',
    deliveryType: (raw.assetModuleMetadata?.deliveryType as GeneratedArtifactAssetSliceSet['deliveryType']) ?? undefined,
    slices: (raw.apkDescription ?? []).map(mapArtifactDescription),
  };
}

function mapTargetingInfo(raw: RawTargetingInfo): GeneratedArtifactTargetingInfo {
  return {
...
export function mapGeneratedArtifactsPerSigningKey(raw: RawPerSigningKey): GeneratedArtifactsPerSigningKey {
  return {
    certificateSha256Hash: raw.certificateSha256Hash ?? '',
    generatedSplitApks: (raw.generatedSplitApks ?? []).map(mapSplitApk),
    generatedStandaloneApks: (raw.generatedStandaloneApks ?? []).map(mapStandaloneApk),
    generatedUniversalApk: raw.generatedUniversalApk
      ? mapUniversalApk(raw.generatedUniversalApk)
      : undefined,
    generatedAssetPackSlices: (raw.generatedAssetPackSlices ?? []).map(mapAssetPackSlice),
    generatedRecoveryModules: (raw.generatedRecoveryModules ?? []).map(mapRecoveryApk),
    targetingInfo: raw.targetingInfo ? mapTargetingInfo(raw.targetingInfo) : undefined,
  };
}
...
export function flattenGeneratedArtifactsToBuildDeliverables(
  ...
  for (const key of signingKeys) {
    ...
    for (const artifact of allArtifacts) {
      const typeSuffix =
        artifact.moduleName ? `-${artifact.moduleName}` :
        artifact.splitId ? `-${artifact.splitId}` :
        '';

      deliverables.push({
        id: artifact.downloadId,
        buildId,
        type: artifact.type === 'split' ? 'app_thinning_variant' : 'app_thinning_variant',
