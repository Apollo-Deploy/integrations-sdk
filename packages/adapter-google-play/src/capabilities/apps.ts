import type { AppStoreCapability, TokenSet, Paginated, StoreApp } from '@apollo-deploy/integrations';
...
export function createGooglePlayApps(
  ...
  return {
    ...
    async getApp(tokens: TokenSet, packageName: string): Promise<StoreApp> {
      return ctx.withEdit(tokens, packageName, async () => mapGoogleApp(packageName, {}));
    },
  };
}
