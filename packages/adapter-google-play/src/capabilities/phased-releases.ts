...
function mapGooglePhasedRelease(
  ...
  return {
    ...
    state,
    rolloutPercentage: state === 'complete'
      ? 100
      : fraction != null
  ...
  return {
    async createPhasedRelease(
      ...
      return ctx.withEdit(tokens, packageName, async (editId) => {
        ...
        const hasMatch = releases.some((r: any) =>
    ...
    async getPhasedRelease(
      ...
      return ctx.withEdit(tokens, packageName, async (editId) => {
        ...
        return mapGooglePhasedRelease(packageName, trackName, versionCode, release);
      });
    },
    ...
    async updatePhasedRelease(
      ...
      return ctx.withEdit(tokens, packageName, async (editId) => {
        ...
        return mapGooglePhasedRelease(packageName, trackName, versionCode, matchedRelease);
      }, { commit: true });
    },
