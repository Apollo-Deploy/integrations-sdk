...
export function createGooglePlayBuilds(
  ...
  AppStoreCapability,
  'listBuilds' | 'getBuild' | 'listBuildArtifacts' | 'getArtifactDownloadUrl' | 'uploadBinary'
> {
  return {
    async listBuilds(
      tokens: TokenSet,
      ...
      return ctx.withEdit(tokens, packageName, async (editId) => {
        const [bundles, apks] = await Promise.all([
          ctx.gpRequest(tokens, `${BASE_URL}/applications/${packageName}/edits/${editId}/bundles`),
          ctx.gpRequest(tokens, `${BASE_URL}/applications/${packageName}/edits/${editId}/apks`),
        ]);
    ...
    async uploadBinary(
      ...
      if (opts.deviceTierConfigId) query.deviceTierConfigId = opts.deviceTierConfigId;

      if (fileType === 'aab') {
        const build = await ctx.withEdit(tokens, packageName, async (editId) => {
          const raw = await ctx.gpUpload<any>(
            tokens,
