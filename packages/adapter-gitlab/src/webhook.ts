...
const GITLAB_EVENT_DOMAIN_MAP: Record<string, 'source-control' | 'issue-tracking' | 'ci-cd'> = {
  Push: 'source-control',
...
export function createGitlabWebhook(_config: GitlabAdapterConfig): WebhookHandler {
  return {
    ...
    verifySignature({ headers, secret }) {
      ...
      try {
        return timingSafeEqual(Buffer.from(token as string), Buffer.from(secret));
      } catch {
        return false;
      }
    },
    ...
    parseEvent({ body, headers }) {
      ...
      const eventName = (headers['x-gitlab-event'] as string) ?? 'unknown';
      const domain = GITLAB_EVENT_DOMAIN_MAP[eventName] ?? 'source-control';
      const actionKind = (b['object_attributes'] as Record<string, unknown>)?.['action'] as string | undefined;
      const eventType = actionKind
      ...
      return {
        ...
        domain,
        eventType,
        timestamp: new Date((b['created_at'] as string) ?? Date.now()),
        correlationId: (headers['x-gitlab-event-uuid'] as string) ?? randomUUID(),
        connectionId: '',
        data: b,
      };
    },
    ...
    getDeliveryId(headers) {
      return (headers['x-gitlab-event-uuid'] as string) ?? `gitlab:${Date.now()}`;
    },
