...
export function createGitlabOAuth(config: GitlabAdapterConfig): OAuthHandler {
  ...
  return {
    getAuthorizationUrl({ state, scopes, redirectUri }) {
      ...
      url.searchParams.set('scope', (scopes.length ? scopes : ['read_user', 'api', 'read_repository']).join(' '));
    ...
    async getIdentity(accessToken) {
      ...
      if (!resp.ok) throw new OAuthError('gitlab', `Failed to fetch identity: ${resp.status}`);
