  ...
  return {
    ...
    verifySignature({ rawBody, headers }) {
      ...
      try {
        ...
        const publicKeyHex = (config.publicKey.match(/.{2}/g) ?? []).map((h) =>
          parseInt(h, 16),
        );
    ...
    parseEvent({ body, headers }) {
      ...
      return {
        ...
        eventType,
        timestamp: new Date(),
        correlationId: headers["x-discord-application-id"] ?? randomUUID(),
        connectionId: "",
        data: b,
      };
    },
    ...
    getDeliveryId(headers) {
      return (
        (headers["x-discord-application-id"] as string | undefined) ??
        `discord:${String(Date.now())}`
      );
    },
