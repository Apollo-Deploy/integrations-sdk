import type { AppStoreCapability, StoreTrack } from '@apollo-deploy/integrations';
...
> {
  const capability: ReturnType<typeof createAppleReleases> = {
    ...
    async listReleases(tokens, appId, opts) {
      ...
      const data = await ctx.appleRequest(tokens, `/appStoreVersions?${params}`);
      return {
    ...
    async listTracks(tokens, appId) {
      const [versions, betaData] = await Promise.all([
        ctx.appleRequest(tokens, `/appStoreVersions?filter[app]=${appId}&limit=1&sort=-createdDate`),
        ctx.appleRequest(tokens, `/preReleaseVersions?filter[app]=${appId}&limit=1`).catch(() => ({ data: [] })),
      ]);
    ...
    async getVersion(tokens, _appId, versionId) {
      const data = await ctx.appleRequest(tokens, `/appStoreVersions/${versionId}`);
    ...
    async submitForReview(tokens, _appId, request) {
      ...
      if (request.phasedRelease) {
        await ctx.appleRequest(tokens, '/appStoreVersionPhasedReleases', {
          method: 'POST',
          body: JSON.stringify({
            data: {
              type: 'appStoreVersionPhasedReleases',
