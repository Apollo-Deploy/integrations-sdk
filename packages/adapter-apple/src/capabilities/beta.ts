...
> {
  return {
    async listBetaGroups(tokens, appId) {
      ...
      const data = await ctx.appleRequest(tokens, `/betaGroups?${params}`);
    ...
    async createBetaGroup(tokens, appId, input) {
      const result = await ctx.appleRequest(tokens, '/betaGroups', {
        method: 'POST',
        body: JSON.stringify({
          data: {
            type: 'betaGroups',
    ...
    async deleteBetaGroup(tokens, _appId, groupId) {
      await ctx.appleRequest(tokens, `/betaGroups/${groupId}`, { method: 'DELETE' });
    },
    ...
    async assignBuildToBetaGroup(tokens, _appId, groupId, buildId) {
      await ctx.appleRequest(tokens, `/betaGroups/${groupId}/relationships/builds`, {
        method: 'POST',
