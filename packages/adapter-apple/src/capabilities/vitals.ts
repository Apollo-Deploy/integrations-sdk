...
function extractApplePerfMetric(
  data: any,
  key: string,
): VitalIndicator {
  if (!data?.data) {
  ...
  const match = (data.data ?? []).find(
    (d: Record<string, any>) =>
      d.attributes?.metricType === key || d.type?.includes(key.toLowerCase()),
  );
  ...
  return {
    async getVitalsSummary(tokens, appId, opts) {
      ...
      const [perfData, _crashData, _hangData] = await Promise.all([
        ctx
          .appleRequest(tokens, `/apps/${appId}/perfPowerMetrics?${params}`)
          .catch(() => null),
        ctx
          .appleRequest(
            tokens,
            `/apps/${appId}/diagnosticSignatures?filter[diagnosticType]=CRASH&limit=1`,
          )
          .catch(() => null),
        ctx
          .appleRequest(
            tokens,
            `/apps/${appId}/diagnosticSignatures?filter[diagnosticType]=HANG&limit=1`,
          )
          .catch(() => null),
      ]);
    ...
    async getVitalMetric(tokens, appId, metric, opts) {
      ...
      const data = await ctx.appleRequest(
        tokens,
        `/apps/${appId}/perfPowerMetrics?${params}`,
      );
    ...
    async listCrashClusters(tokens, appId, opts) {
      ...
      return {
        items: (data.data ?? []).map((s: Record<string, any>) =>
          mapAppleCrashCluster(appId, s as any),
        ),
    ...
    async getCrashCluster(tokens, _appId, clusterId) {
      const [signature, logs] = await Promise.all([
        ...
        ctx
          .appleRequest(tokens, `/diagnosticSignatures/${clusterId}/logs`)
          .catch(() => ({ data: [] })),
      ]);

      const cluster = mapAppleCrashCluster("", signature.data);
      cluster.stackTrace = logs.data?.[0]?.attributes?.diagnosticLog as
        | string
        | undefined;
      return cluster;
    },

    async listAnrClusters(): Promise<Paginated<AnrCluster>> {
      // ANR is Android-only. Apple equivalent is "hangs" — use listCrashClusters with HANG type.
      return Promise.resolve({ items: [] as AnrCluster[], hasMore: false });
    },
  };
}
