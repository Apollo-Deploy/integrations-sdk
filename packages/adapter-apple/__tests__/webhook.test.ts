...
describe('createAppleWebhook', () => {
  const handler = createAppleWebhook({ issuerId: 'i', keyId: 'k', privateKey: '', webhookSecret: WEBHOOK_SECRET });

  test('verifySignature accepts valid HMAC-SHA256 signature', () => {
    const rawBody = Buffer.from('{"type":"BUILD_UPLOAD_STATE_CHANGE","data":{}}');
  ...
  test('parseEvent maps BUILD_UPLOAD_STATE_CHANGE to build.status_changed', () => {
    const body = {
      ...
      data: { id: 'build-123', type: 'builds', attributes: { processingState: 'VALID' } },
    };
  ...
  test('getDeliveryId returns x-apple-delivery-id when present', () => {
    const id = handler.getDeliveryId({ 'x-apple-delivery-id': 'delivery-abc' }, {});
