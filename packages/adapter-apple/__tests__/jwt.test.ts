...
describe('generateAppleJWT', () => {
  ...
  test('payload contains iss, iat, exp, and aud', () => {
    const { privateKey } = generateTestKeyPair();
    const config = { issuerId: 'MY-ISSUER-ID', keyId: 'ABCDEF1234', privateKey };
  ...
  test('different issuerIds produce different JWTs', () => {
    const { privateKey } = generateTestKeyPair();
    const jwt1 = generateAppleJWT({ issuerId: 'issuer-1', keyId: 'KEY1', privateKey });
    const jwt2 = generateAppleJWT({ issuerId: 'issuer-2', keyId: 'KEY1', privateKey });
