...
import type { WebhookHandler, SynchronousResponse } from '@apollo-deploy/integrations';
...
export function createSlackWebhook(config: SlackAdapterConfig): WebhookHandler {
  return {
    ...
    verifySignature({ rawBody, headers, secret }) {
      const signingSecret = secret || config.signingSecret;
    ...
    parseEvent({ body }) {
      ...
      const eventType = outerType === 'event_callback'
        ? ((b['event'] as Record<string, unknown>)?.['type'] as string) ?? outerType
        : outerType;
    ...
    getDeliveryId(headers, body) {
      ...
      const eventId = (b['event_id'] as string) ?? (b['event'] as any)?.['event_ts'];
